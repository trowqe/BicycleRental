...
public class AddBicycleCommandTest {
	...
	@InjectMocks
	private AddBicycleCommand command;
	
	@Before
	public void setup() throws ServiceException {
		when(request.getSession()).thenReturn(session);
		when(session.getAttribute(SessionAttributes.LANGUAGE)).thenReturn(LANGUAGE);
		when(request.getParameter(AddBicycleCommand.RENTAL_POINT_ID_PARAM)).thenReturn("1");
		when(request.getParameter(AddBicycleCommand.BICYCLE_MODEL_ID_PARAM)).thenReturn("1");
	}
	
	@Test
	public void shouldReturnForwardResponseToBicyclesCommandAndSuccessMessage() throws CommandException, ServiceException {
		doNothing().when(service).createByPointAndModelId(anyLong(), anyLong());
	...
	public void shouldReturnThrowCommandExceptionWhenRentalPointServiceThrowServiceException() throws ServiceException, CommandException {
		doThrow(new ServiceException()).when(service).createByPointAndModelId(anyLong(), anyLong());
